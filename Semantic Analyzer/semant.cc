...
ClassTable::ClassTable(Classes classes) : semant_errors(0) , error_stream(cerr) {
    ...
    {
        //get name of classe getName of classes->nth(i)
        //in map put classMap[getName]=classes->nth(i);;
    ...
    {
        if(!cycle.empty() && classes->nth(i)->getParent() == Object )
        {
            cycle.clear();
        }
        else
    ...
    /*3. For each class collect information about all attributes and methods in SymbolTable in the class*/ 
    for(int i = classes->first(); classes->more(i); i = classes->next(i))
    {
        //for any ith class go through the feature list
        Features features = classes->nth(i)->getFeatures();
        for(int i = features->first(); features->more(i); i = features->next(i))
        {
            features->nth(i)->dump(std::cout, 1);

            //if feature is an attribute
            if(features->nth(i)->getFormals() == nil_Formals())
            {
                classes->nth(i)->attrStore.addid(classes->nth(i)->attrStore->getName(), classes->nth(i)->attrStore);
            }
            //if feature is a method
        }
    } 
